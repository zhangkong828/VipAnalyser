...
namespace VipAnalyser.LoginManager
{
    public class ChromeDriverHelper
    {
        ...
        public ChromeDriverHelper(string cookies, string domain)
        {
            var _option = new ChromeOptions();
            _option.AddArgument("ignore-certificate-errors");
            //_option.AddArgument("--headless");
            _option.AddArgument("--disable-gpu");
            _option.AddArgument("--user-agent=Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/50.0.2661.102 Safari/537.36");

            var _service = ChromeDriverService.CreateDefaultService();
            ...
            _webDriver = new ChromeDriver(_service, _option);
        ...
        public bool QQLogin(string username, string password, out string cookies)
        {
            ...
            Thread.Sleep(2000);
            //等待 qq登录frame
            if (!WaitForElementExists(By.Id("login_win_type"), 30))
            {
